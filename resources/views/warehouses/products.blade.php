            ...
            <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8">
                    <div class="mb-6 sm:mb-0">
                        <h1 class="text-4xl font-bold text-slate-900">Products in Warehouse</h1>
                        <p class="text-slate-600 mt-2 text-lg">{{ $warehouse->WarehouseName }} - Manage products and inventory</p>
                    </div>
                ...
                <div class="bg-white shadow-lg rounded-xl border border-slate-200 overflow-hidden">
                                    ...
                                    <tr
                                        ...
                                        <td class="px-6 py-4 text-right">
                                            <div class="flex justify-end gap-2">
                                                ...
                                                <form method="POST" action="{{ route('warehouses.products.update', ['warehouse' => $warehouse, 'product' => $product]) }}" class="inline">
                                                    @csrf
                                                    @method('PATCH')
                                                ...
                                                <!-- Remove Form -->
                                                <form method="POST" action="{{ route('warehouses.products.remove', ['warehouse' => $warehouse, 'product' => $product]) }}" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button onclick="return confirm('Remove this product from the warehouse?')"
                                    ...
                                    <tr class="sm:hidden">
                                        <td class="p-0">
                                            <div class="p-4 border-b border-slate-200 hover:bg-slate-50 transition-colors">
                                                ...
                                                <!-- Update Quantity Form -->
                                                <form method="POST" action="{{ route('warehouses.products.update', ['warehouse' => $warehouse, 'product' => $product]) }}" class="mb-2">
                                                    @csrf
                                                    @method('PATCH')
                                                ...
                                                <!-- Remove Form -->
                                                <form method="POST" action="{{ route('warehouses.products.remove', ['warehouse' => $warehouse, 'product' => $product]) }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button onclick="return confirm('Remove this product from the warehouse?')"
                    ...
                    @else
                        <div class="p-12 text-center">
                            <p class="text-slate-600 text-lg mb-4">No products found in this warehouse.</p>
                            <a href="{{ route('warehouses.products.assign.page', $warehouse) }}"
